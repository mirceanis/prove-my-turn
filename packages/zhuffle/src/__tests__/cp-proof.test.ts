...
import { generateCPProof, verifyCPProof } from '../cp-proof';
import { CPProof, PRNG } from '../types';
...
describe('Chaum-Pedersen proofs', () => {
  describe('generateCPProof', () => {
    ...
    it('should generate different proofs for the same inputs (due to randomness)', () => {
      ...
      const x = curve.mul(g, alfa);
      const y = curve.mul(h, alfa);
      const fsprng1 = new KeccakFSRNG();
      const fsprng2 = new KeccakFSRNG();
  ...
  describe('verifyCPProof', () => {
    it('should verify a valid proof', () => {
      ...
      const verifierFsprng = new KeccakFSRNG();
    ...
    it('should work with multiple valid proofs in sequence', () => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        const proof = generateCPProof(curve, x, y, g, h, alfa, new KeccakFSRNG(), rng);

        const isValid = verifyCPProof(curve, proof, x, y, g, h, new KeccakFSRNG());
